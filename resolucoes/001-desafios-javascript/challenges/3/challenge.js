...
const doRequest = () => {
    // api retona os dados, assim serão carregados
    api().then(data => {
        setLoading(true) 
        setError(false)
        return data
    ...
    .then(data => {
        setData(data) // dados carregados - termina o loading
        setLoading(false)
    })
    .catch(e => setError(true)) // se somente se houver, setErrot será true
}
